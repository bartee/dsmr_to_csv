...
while successful != True:
    try:
        ...
        for telegram in serial_reader.read():
            ...
            message_datetime = telegram[
                obis_references.P1_MESSAGE_TIMESTAMP
            ].value.strftime("%Y%m%d-%H:%I:%S")
            ...
            electricity_used_total = telegram[
                obis_references.ELECTRICITY_USED_TARIFF_ALL[active_tariff - 1]
            ].value
            electricity_delivered_total = telegram[
                obis_references.ELECTRICITY_DELIVERED_TARIFF_ALL[active_tariff - 1]
            ].value
            ...
            resultset = [
                message_datetime,
                active_tariff,
                str(electricity_used_total),
                str(electricity_delivered_total),
                str(gas_reading),
            ]
...
print(
    "Finished after trying {counter} times within {elapsed_time:0.4f} sec".format(
        counter=counter, elapsed_time=elapsed_time
    )
)
